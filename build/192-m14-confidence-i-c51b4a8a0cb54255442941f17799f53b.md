...
```{admonition} Example
    ...
    We first need to calculate the sample proportion $\overset{ˆ}{p}$. Out of 1,005 sampled adults, 643 knew what Viagra is used for, so $\overset{ˆ}{p}=\frac{643}{1005}=.64$
    ...
    A 95% confidence interval for p is $\overset{ˆ}{p}\pm2\sqrt{\frac{\overset{ˆ}{p}\left(1−\overset{ˆ}{p}\right)}{n}}=.64\pm2\sqrt{\frac{.64\left(1−.64\right)}{1005}}=.64\pm.03=\left(.61, .67\right)$
