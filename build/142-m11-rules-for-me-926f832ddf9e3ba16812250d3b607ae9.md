...
If X is a random variable with the mean $μ_{X}$ and a variance of $σ_{X}^{2}$, then the new random variable *a + bX* has a mean and variance (respectively) of:

$μ_{a+bX}=a+bμ_{X}$

$σ_{a+bX}^{2}=b^{2}σ_{X}^{2}$
...
```{admonition} Example: Shifting and Stretching
    ...
    $μ_{50+5X}=50+5μ_{x}=50+5(1.8)=59$

    $σ_{50+5X}^{2}=5^{2}σ_{X}^{2}=25(1.46)=36.5$

    and therefore: $σ_{50+5X}=\sqrt{36.5}=6.04$
