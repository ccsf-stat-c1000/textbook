...
Since $m=z\times\frac{σ}{\sqrt{n}}$ is the formula to determine m for a given n, we can use simple algebra to express n in terms of m (multiply both sides by the square root of n, divide both sides by m, and square both sides) to get
...
```{admonition} Example
    ...
    $n=\left(\frac{z^{*}σ}{m}\right)^{2} = \left(\frac{2.576\left(15\right)}{2}\right)^{2} = 373.26$
