...
In the case of hypothesis testing for the population proportion (p), we will collect data on the relevant categorical variable from the individuals in the sample and start by calculating the sample proportion, $\overset{ˆ}{p}$ (the natural quantity to calculate when the parameter of interest is p).
...
The test statistic is *a measure* of how far the sample proportion $\overset{ˆ}{p}$ is from the null value $p_{0}$, the value that the null hypothesis claims is the value of p. In other words, since $\overset{ˆ}{p}$ is what the data estimates p to be, the test statistic can be viewed as a measure of the "distance" between what the data tells us about p and what the null hypothesis claims p to be.
...
```{admonition} Example: 1
    ...
    The data estimate p to be $\overset{ˆ}{p}=.16$
...
```{admonition} Example: 2
    ...
    The data estimate p to be $\overset{ˆ}{p}=.19$.
...
```{admonition} Example: 3
    ...
    The data estimate p to be $\overset{ˆ}{p}=.675$
...
There is a problem with just looking at the difference between the sample proportion $\overset{ˆ}{p}$ and the null value $p_{o}$.
