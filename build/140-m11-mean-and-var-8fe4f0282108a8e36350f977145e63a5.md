...
```{admonition} Example: Xavier's and Yves' Production Lines
    ...
    Therefore, $σ_{Y}<σ_{X}=1.21$ Actually, $σ_{Y}=0.85$, so we can draw the following conclusion:
...
```{admonition} Example: Xavier's Production
    ...
    We know that $μ_{X}=1.8$ and $σ_{X}=1.21$.
...
```{admonition} Example: Comparing Investments
    ...
    $μ_{Y}=0(0.98)+1000000(0.02)=20000$

    $μ_{z}=10000(0.5)+20000(0.5)=15000$
    ...
    $σ_{x}^{2}=(14000-14000)^{2}(1)=0$

    $σ_{x}=0$

    $σ_{Y}^{2}=(0-20000)^{2}(0.98)+(1,000,000-20000)^{2}(0.02)=1.96×10^{10}$

    $σ_{Y}=140,000$

    $σ_{z}^{2}=(10000-15000)^{2}(0.5)+(20000-15000)^{2}(0.5)=25,000,000$
