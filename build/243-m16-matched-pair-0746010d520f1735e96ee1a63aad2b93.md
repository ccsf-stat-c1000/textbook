...
```{admonition} Example
    ...
    1. *Stating the hypotheses.* Recall that in matched pairs, we reduce the data from two samples to one sample of differences: and we state our hypotheses in terms of the mean of the differences, $μ_{d}$. Since we would like to test whether there are differences in IQ scores between people who were raised by their birth parents and those who weren't, we are carrying out the two-sided test: *Comment:* Again, some students find it easier to first think about the hypotheses in terms of μ~1~ and μ~2~, and then write them in terms of $μ_{d}$. In this case, since we are testing for differences between the two populations, the hypotheses will be: and since $μ_{d}=μ_{1}−μ_{2}$ we get back to the hypotheses above.
