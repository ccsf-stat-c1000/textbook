...
```{admonition} Example
    ...
    Suppose a poll of 1,000 U.S. adults finds that 560 of them believe marijuana should be legalized. If we wanted to estimate p, the population proportion, using a single number based on the sample, it would make intuitive sense to use the corresponding quantity in the sample, the sample proportion $\overset{ˆ}{p}=\frac{560}{1000}=.56$. We say in this case that .56 is the *point estimate* for p, and in general, we'll always use $\overset{ˆ}{p}$ as the *point estimator* for p. (Note, again, that when we talk about the *specific value* (.56), we use the term estimate, and when we talk in general about the *statistic* $\overset{ˆ}{p}$, we use the term *estimator.*Here is a visual summary of this example:
