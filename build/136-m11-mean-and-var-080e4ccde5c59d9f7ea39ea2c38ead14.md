...
Whereas we denoted the mean of a sample as $\bar{x}$, we now denote the mean of a random variable as $μ_{X}$. Let's see how this is done by looking at a specific example.
...
```{admonition} Example: Xavier's Production Line
    ...
    $μ_{X}=0(0.15)+1(0.30)+2(0.25)+3(0.20)+4(0.10)=1.8$
```
