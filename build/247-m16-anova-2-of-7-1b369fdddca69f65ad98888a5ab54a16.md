...
The null hypothesis claims that there is no relationship between X and Y. Since the relationship is examined by comparing$μ_{1},μ_{2},μ_{3},...,μ_{k}$, the means of Y in the populations defined by the values of X, no relationship would mean that all the means are equal. Therefore the null hypothesis of the F-test is: $H_{0}:μ_{1}=μ_{2}=...=μ_{k}$

As we mentioned earlier, here we have just one alternative hypothesis, which claims that there *is* a relationship between X and Y. In terms of the means $μ_{1},μ_{2},μ_{3},...,μ_{k}$, it simply says the opposite of the alternative, that not all the means are equal, and we simply write:$H_{a}:not all the μ's are equal$.
...
```{admonition} Example
    ...
    Note that there are many ways for $μ_{1},μ_{2},μ_{3},μ_{4}$ not to be all equal, and	$μ_{1}\neqμ_{2}\neqμ_{3}\neqμ_{4}$	is just one of them. Another way could be	$μ_{1}=μ_{2}=μ_{3}\neqμ_{4}$ or	$μ_{1}=μ_{2}\neqμ_{3}=μ_{4}$. The alternative of the ANOVA F-test simply states that not all of the means are equal, and is not specific about the way in which they are different.
```
