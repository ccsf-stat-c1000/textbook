...
$σ_{X}^{2}=np(1-p);σ_{X}=\sqrt{np(1-p)}$
...
$μ_{X}=x_{1}p_{1}+x_{2}p_{2}+...+x_{n}p_{n}=\sumi=1nx_{i}p_{i}$
...
$μ_{X}=np$ and $σ_{X}^{2}=np(1-p);σ_{X}=\sqrt{np(1-p)}$ than it would be to calculate the mean and variance or standard deviation from scratch.
...
```{admonition} Example: Blood Type B—Standard Deviation
    ...
    $σ_{X}^{2}=120(0.1)(1-0.1)=10.8;σ_{X}=\sqrt{10.8}\approx3.3$
