...
2. We typically estimate: Note that the last two parameters (σ and $σ^{2}$ ) are not covered in this course.
  - the population mean *μ* by the sample mean $\bar{x}$
  - the population proportion *p* by the sample proportion $\overset{ˆ}{p}$
  - the population standard deviation *σ* and the population variance $σ^{2}$ by the sample standard deviation *s* and the sample variance $s^{2}$
3. $\bar{x}$, $\overset{ˆ}{p}$, s, and $s^{2}$ are unbiased estimators for μ, p, σ, and $σ^{2}$, respectively. Their precision increases with the sample size.
...
6. The confidence interval for the population proportion p is the primary statistical method used in the analysis of polls, and can be safely used as long as $n⋅\overset{ˆ}{p}\geq10$ and $n⋅\left(1−\overset{ˆ}{p}\right)\geq10$.
