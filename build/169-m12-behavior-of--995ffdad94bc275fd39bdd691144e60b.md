...
So far, we've discussed the behavior of the statistic $\overset{ˆ}{p}$, the sample proportion, relative to the parameter p, the population proportion (when the variable of interest is categorical). We are now moving on to explore the behavior of the statistic $\bar{X}$, the sample mean, relative to the parameter $μ$, the population mean (when the variable of interest is quantitative).

## Behavior of Sample Mean¯¯¯X
